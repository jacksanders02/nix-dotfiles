...
#     "$SHUFFLE_ICON"
magick -size "${THUMBNAIL_WIDTH}x${THUMBNAIL_HEIGHT}" xc:transparent \
    \( "$HOME/.config/wofi/wallpaper-picker/assets/shuffle.png" -resize "80x80" \) \
...
generate_menu() {
    ...
    for img in "$WALLPAPER_DIR"/*.{jpg,jpeg,png}; do
        ...
        if [[ ! -f "$thumbnail" ]] || [[ "$img" -nt "$thumbnail" ]]; then
            notify-send "Generating Thumbnail" "$thumbnail"
